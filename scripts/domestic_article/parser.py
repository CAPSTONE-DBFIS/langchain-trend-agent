...
def parse_data(article_info):
    """
    article_info: dict 형태, {"url": ..., "category": ...}
    """
    url = article_info["url"]
    provided_category = article_info["category"]
    ...
    # 대신 Scraper에서 넘겨준 category 사용
    category = provided_category

    # 언론사, 제목, 날짜, 내용, 이미지 추출 (기존 로직)
    ...
    return {
        "category": category,  # Scraper에서 넘겨준 값을 그대로 사용
